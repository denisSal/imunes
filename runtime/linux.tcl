...
proc isNodeIfacesCreated { node_id ifaces } {
	...
	set cmds ""
	foreach iface_id $ifaces {
		append cmds "ip link show [getIfcName $node_id $iface_id] > /dev/null 2>/dev/null && "
	}
	...
	try {
		if { $ifacesconf_timeout >= 0 } {
			rexec timeout [expr $ifacesconf_timeout/5.0] docker exec -t $docker_id sh -c {*}$cmds
		} else {
			rexec docker exec -t $docker_id sh -c {*}$cmds
		}
	} on error {} {
		return false
	}
