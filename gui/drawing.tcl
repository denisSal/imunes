proc refreshToolBarNodes {} {
	...
	foreach node_type $all_modules_list {
		...
		if { $tool == "" } {
			sputs "No toolbarLocation defined for '$node_type', skipping."
		...
		$mf.left.${tool}_nodes add command -image $image -hidemargin 1 \
			-compound left -label [invokeTypeProc $node_type "gui::toolbarIconDescr"] \
...
proc snapCoordsToGrid { x y } {
	...
	set snapped_x [expr { round($x / $grid) * $grid }]
