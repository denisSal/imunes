	...
	foreach eid $running_eids {
		...
		if { $link_id != "" } {
			...
			foreach cur_link_id $cur_link_list {
				...
				if { $iface1_id != "" } {
					if { $iface1_id != $cur_iface1_id } {
						continue
					}
				}

				if { $iface2_id != "" } {
					if { $iface2_id != $cur_iface2_id } {
						continue
					}
				}
			...
			if { [llength $node_pairs] == 0 } {
				return -code error "No links with given peers '$link_idname' were found in experiment $eid"
			}

			if { [llength $node_pairs] > 1 } {
				return -code error "Multiple links with same peers '$link_idname' ([join $node_pairs ", "]) were found in experiment $eid"
			}
